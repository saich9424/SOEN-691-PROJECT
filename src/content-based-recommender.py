...
# updated_ratings_df.write.csv('updated_ratings_df.csv')
# reviews_restaurant_df.write.csv('reviews_restaurant_df.csv')
# norm.write.csv('norm.csv')
# temp_restaurant_df.write.csv('temp_restaurant_df.csv')
#
# # categories_set = categories_rdd.reduce(lambda x, y: x.union(y))
#
# # user id 0's profile
# uid_0_user_profile = updated_ratings_df.where('user_index == 0')
#
# #  Get all businesses corresponding to the required user and make list of those businesses
# bid_with_our_user = reviews_restaurant_df.where("user_index == 0")
# list_bid_our_user = bid_with_our_user.select('restaurant_index').collect()
# list_bid_our_user_modified = []
# for item in list_bid_our_user:
#     list_bid_our_user_modified.append(item.restaurant_index)
#
# rest_business_weighted_score_df = norm.rdd.filter(lambda x: x.restaurant_index not in list_bid_our_user_modified).toDF()
#
# user_0_predictions = rest_business_weighted_score_df.crossJoin(uid_0_user_profile)
#
#
# def get_dot_product_user_0(x):
#     row_dict = x.asDict()
#     bid = row_dict.pop('restaurant_index')
#     uid = row_dict.pop('user_index')
#
...
#     weighted_score_squared_sum = math.sqrt(weighted_score_squared_sum)
#     user_profile_squared_sum = math.sqrt(user_profile_squared_sum)
#     prediction = prediction / (weighted_score_squared_sum * user_profile_squared_sum)
#     prediction = (((prediction - (-1)) / (1 - (-1))) * (5 - 1)) + 1
#     output = {'user_index': uid, 'restaurant_index': bid, 'prediction': prediction}
#     output = Row(**output)
#     return output
#
#
# user_0_predictions = user_0_predictions.rdd.map(lambda x: get_dot_product_user_0(x)).toDF()
# user_0_predictions = user_0_predictions.join(temp_restaurant_df, 'restaurant_index').orderBy('prediction',
#                                                                                            ascending=False)
# user_0_predictions = user_0_predictions.select('user_index', 'restaurant_index', 'name', 'prediction', 'TotalReviews',
#                                                'user_rating',
#                                                'categories')
# user_0_predictions = user_0_predictions.withColumnRenamed('user_rating', 'business_rating')
